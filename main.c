//********************************************************************************//
//*************STAND ALONE SINGLE PHASE HALF-BRIDGE INVERTER**********************// 
    ...
    if(current_Flag) //20Khz
       {  
          ...
          if(Ivalue < current_min) fault_Flag = 0; //min current trip value  
            ...
            asmPWM(); //generate Half-bridge inverter duty cycle          
	...
	if(pll_Flag) //12Khz _PLL_count
		{
          ...
          asmABCtoDQ(); //converte current feedback to d-q format
		...
		if(ffd_Flag) //0.5Khz   
              ...
              V_ref++; //initiate soft start
                   ...
                   if(V_ref >= V_Dsetpoint)
                       { 
                         V_ref = V_Dsetpoint; //set voltage reference
